...
class Article:
    ...
    async def fetch_metadata(self, doi=None, pmid=None, title=None):
        async with aiohttp.ClientSession as session:
            ...
            if manifest_path.exists():
                with aiofiles.open(str(manifest_path),
            ...
            if self.doi is not None:
                ...
                async with session.get(url, headers=headers) as response:
                    self.update_metadata(response.content)
    ...
    async def update_manifest(self):
        manifest_path = self.path.joinpath('manifest.json')
        async with aiofiles.open(str(manifest_path,'w', encoding='utf-8') as m:
    ...
    async def update_meta_db(self, entry):
        async with aiofiles.open('database.json', 'r') as f:
            db = json.loads(await f.read())
        if 'doi' in entry:
            db_entry = db['doi'][entry['doi']]
            db_entry = update_dbentry(self, db_entry, ref)
            db['doi'][entry['doi']] = db_entry
        if 'pmid' in entry:
            db_entry = db['pmid'][entry['pmid']]
            db_entry = update_dbentry(self, db_entry, ref)
            db['pmid'][entry['pmid']] = db_entry
        if 'title' in entry:
            db_entry = db['title'][entry['title']]
            db_entry = update_dbentry(self, db_entry, ref)
